...
function initGlobalUI() {
    ...
    // Ensure Ripple Style Exists once
...
function attachRipples() {
    document.querySelectorAll('.nav-icon, .menu-btn, .sidebar-item, .action-btn').forEach(btn => {
...
function initHomePage() {
    ...
    // Sidebar Active State & Content Simulator
...
function initWatchPage() {
    ...
    const videoId = urlParams.get('id') || 'vid1'; // fallback to first video
    ...
    if (likeBtn) {
        likeBtn.addEventListener('click', function() {
            ...
            if (this.classList.contains('liked')) {
                this.style.color = '#3ea6ff'; // YouTube-like blue tint on dark mode when liked
...
function loadVideoData(id) {
    ...
    document.getElementById('mainVideoPlayer').load(); // Reload video element
    ...
    // Reset liked state if navigating dynamically
    const likeBtn = document.getElementById('likeBtn');
    if (likeBtn) {
    ...
    // Populate recommendations (exclude current)
    ...
    recommendations.forEach(rec => {
        ...
        div.addEventListener('click', () => {
            // Update URL cleanly without reloading page using History API
    ...
    // Ensure ripples are attached to dynamic elements
    attachRipples();
}